...
export const cameraHandlers = [
  ...
  http.post('/plan/report/send', async ({ request }) => {
    ...
    try {
      ...
      // 新しいAPI仕様に準拠したレスポンス（ローカル環境用）
      const response = {
        report_id: reportId,
        kind: kind,
        // new_subscription: isNewReport && kind === 'before', // Before撮影 かつ 新規レポートの場合のみtrue
        ...
        return_base_url: `http://localhost:3000/mypage/plan/detail/5` // ローカル開発環境用
      };
