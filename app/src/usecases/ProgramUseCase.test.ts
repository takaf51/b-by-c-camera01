...
import type { Program, ProgramListRequest, ProgramListResponse, ProgramError } from '../domain/program';
...
describe('ProgramUseCase', () => {
  ...
  describe('getProgramList', () => {
    it('デフォルト値を設定してリポジトリを呼び出す', async () => {
      vi.mocked(mockRepository.getProgramList).mockResolvedValue(mockListResponse);
    ...
    it('同じステータスの場合は開始日でソートする', async () => {
      const programsWithSameStatus: Program[] = [
        { ...mockPrograms[0], status: 'active', startDate: '2024-01-03T10:00:00Z' },
        { ...mockPrograms[1], status: 'active', startDate: '2024-01-01T10:00:00Z' },
        { ...mockPrograms[2], status: 'active', startDate: '2024-01-02T10:00:00Z' }
      ];
      ...
      vi.mocked(mockRepository.getProgramList).mockResolvedValue(responseWithSameStatus);
    ...
    it('リポジトリエラーを適切に処理する', async () => {
      ...
      vi.mocked(mockRepository.getProgramList).mockRejectedValue(repositoryError);
  ...
  describe('getProgramDetail', () => {
    ...
    it('リポジトリエラーを適切に処理する', async () => {
      ...
      vi.mocked(mockRepository.getProgramDetail).mockRejectedValue(repositoryError);

      await expect(useCase.getProgramDetail(1)).rejects.toEqual(repositoryError);
    });
  });
});
